...
이 저장소의 현재 운영 정본은 **Story Chain + Mission Control + engineering assurance**다. 새 제품 개발은 Story Chain 계약을 따른다.
...
5. **구현** — 제품 코드는 `app/`·`src/`에 둔다. 콘텐츠 검증은 `assets/content/`와 `scripts/content/check.mjs`를 기준으로 한다.
...
- **Never**: 출처 없는 감각 콘텐츠 승격 / weak→strong 무단 승격 / 훈련·전이 문장 중복 / 인식 통계와 산출 통계 병합 / 게이미피케이션 중심 확장 / 범용 영어앱 확장.
