...
- blocked_until: R10 candidate exists, scripts target R10, monitor outputs are received or cycle remains open, assets recover R10 lessons, and `npm run quality:check` passes
...
- blocked_until: paid-proof surface supports the macro bottleneck promise instead of replacing it
...
- required_gates: macro map primary, highest bottleneck, propagation path, current/future visibility, paid-proof layer subordinate
- blocked_until: first viewport answers the bottleneck question before paid-proof capture
...
- blocked_until: `scripts/check-data-contracts.mjs` passes R10

- skill: `ai-infra-bottleneck-radar/skills/feedback-ready-product-surface/SKILL.md`
- loaded_at_step: r10-paid-proof-start
- required_gates: repeat-use workflow, pricing evaluation, local paid-proof packet, report copy, JSON export, macro promise preservation
- blocked_until: paid-proof capture remains local evidence and does not claim paid launch proof
...
- current_candidate_id: `r10-paid-proof-radar`
- current_candidate_path: `candidates/r10-paid-proof-radar/`
...
- current promise: R10 preserves the AI infrastructure macro bottleneck map and adds a subordinate paid-proof local context for monthly radar, sample paid report, and exportable intent evidence.
- primary surface: investor-facing macro bottleneck map
- forbidden drift: paid-proof-only page, waitlist-only page, stock picker, company cards, buy/sell signal, target price view

## Candidate Disposition

- local_candidate_status: pass
- representative_status: pass-as-paid-proof-local-candidate
- sellable_status: blocked
- next_action: create `candidates/r11-external-proof-radar/` as a fresh candidate; do not patch R10
- next_candidate_id: r11-external-proof-radar
- next_candidate_path: candidates/r11-external-proof-radar/
- next_candidate_primary_advancement: external-proof readiness
- allowed_to_stop: no
- stop_permission_after_r10: denied
...
- final_permission_next_action: create `candidates/r11-external-proof-radar/` as a fresh candidate
- stop_permission_reason: R10 strengthens the paid journey locally, but still lacks external paid proof, approved payment/legal boundary, and staffed update SLA.
...
- R10 fresh candidate added under `candidates/r10-paid-proof-radar/`
- R10 turns local subscription intent into a paid-proof packet with customer interview, waitlist, or payment-intent export path.
- R10 keeps paid-proof controls below the macro map and labels them local evidence, not paid launch proof.
- `check-data-contracts.mjs` now checks R10 paid-proof UI, proof boundary, external evidence path, local storage, and macro-support copy.
...
- R10 learnings are recovered into product contract, data operations, visualization UX, business logic, process monitoring, skill registry, and current-candidate scripts.
...
- product_level_verdict: paid-proof local candidate but continue toward external paid proof and approved operation
...
- cycle gate: must pass only when exact R10 current candidate fields, monitor receipt fields, and product-level verdict fields are recorded
...
- HTTP check: required for local smoke at `http://localhost:4173/ai-infra-bottleneck-radar/candidates/r10-paid-proof-radar/`
...
- Data/Sellability: continue-to-r11
...
- current_cycle_promise: R10 preserves the macro bottleneck map first and adds subordinate paid-proof local evidence export for customer interview, waitlist, or payment-intent testing.
...
- narrowed: cycle focus shifted toward paid-proof packet, pricing intent, contact capture, and external evidence export; these are local proof-support controls, not the primary product promise.
- drift_risk: medium
- required_gate_change: paid-proof UI is valid only if the first viewport first answers highest bottleneck, propagation path, current/future change, and source/trust boundary for ordinary investors before any paid-proof or conversion control is evaluated.
- verdict: preserve

### Process Improvement

- process_learning: R10 correctly records Process Improvement Monitor receipt and `quality:final` stop denial; the loop now distinguishes valid current candidate from permission to stop.
...
- mechanical_verdict_gap: repaired by making `check-cycle-record.mjs` bind R10 non-sellable cycles to `r11-external-proof-radar`.
- next_cycle_process_rule: R11 cannot start or verdict unless scripts target `r11-external-proof-radar`, R10 remains archive-only, and cycle record exact next-candidate fields match the final permission next action.
- verdict: repair-before-next, recovered before verdict

### Asset Steward

- asset_map_present: true
- asset_categories_checked: product contract, skill registry, product-making skills, research/data ops, visualization/UX, business logic, process monitoring, quality scripts, R10 candidate
...
- cycle_contributions: R10 added `assets/customer-evidence.md`; mapped it in `ASSET_MAP.md`; added paid-proof local acceptance to product contract; added R10 paid-proof data rule; added R10 paid-proof UX rule; added R10 sellability verdict; added R10 process rule; registered paid-proof packet behavior under feedback-ready surface skill.
- missing_or_stale_assets: no R10-blocking stale assets found; customer evidence is local-only by design; external customer/payment/legal/SLA proof is still missing and correctly treated as next-cycle evidence, not R10 proof.
- next_cycle_asset_rule: R11 must turn the paid-proof packet into an external evidence capture path, or explicitly refuse external proof and keep sellability blocked with product-level reason.
- verdict: keep
...
- sellability_gap: medium; local paid-proof packet exists but external paid proof is still missing.
...
- stop_or_continue: continue-to-r11

## R11 Required Rule

R11 must create a new candidate folder. It must not patch `candidates/r10-paid-proof-radar/`. Its main advancement must be external-proof readiness: a real customer interview/waitlist/payment-intent capture path or an explicit refusal that keeps sellability blocked, plus continued macro bottleneck map preservation.
