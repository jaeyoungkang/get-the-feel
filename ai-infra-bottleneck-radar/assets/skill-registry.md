...
| --- | --- | --- | --- | --- |
| `skills/radar-cycle/SKILL.md` | Every fresh candidate cycle. | candidate id, previous disposition, current-candidate gates, blockers. | `scripts/check-assets.mjs`, `scripts/check-cycle-record.mjs`, `npm run quality:check`. | R13 adds monitor `next_candidate_binding_checked` to prevent stale self-contradictory next-candidate text. |
...
| `skills/product-level-sellability/SKILL.md` | Before writing `sellable_status` or `allowed_to_stop`. | product feedback readiness, repeat-use readiness, pricing evaluation readiness, macro promise preservation. | `scripts/check-cycle-record.mjs` requires product-level verdict fields. | R13 keeps sellability blocked while operator SLA is not staffed. |
