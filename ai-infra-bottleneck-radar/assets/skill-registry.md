...
| --- | --- | --- | --- | --- |
| `skills/radar-cycle/SKILL.md` | Every fresh candidate cycle. | candidate id, previous disposition, current-candidate gates, blockers. | `scripts/check-assets.mjs`, `scripts/check-cycle-record.mjs`, `npm run quality:check`. | R10 keeps monitor output receipt and next fresh candidate naming as cycle gates. |
