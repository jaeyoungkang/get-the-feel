...
| --- | --- | --- | --- | --- |
| `skills/radar-cycle/SKILL.md` | Every fresh candidate cycle. | candidate id, previous disposition, current-candidate gates, blockers. | `scripts/check-assets.mjs`, `scripts/check-cycle-record.mjs`, `npm run quality:check`. | R9 requires the next fresh candidate to be named when final permission is denied. |
