...
| --- | --- | --- | --- | --- |
| `skills/radar-cycle/SKILL.md` | Every fresh candidate cycle. | candidate id, previous disposition, current-candidate gates, blockers. | `scripts/check-assets.mjs`, `scripts/check-cycle-record.mjs`, `npm run quality:check`. | R14 keeps monitor `next_candidate_binding_checked` active for R15 binding. |
...
| `skills/feedback-ready-product-surface/SKILL.md` | Before customer proof, onboarding, pricing, waitlist, or feedback UI. | feedback capture, repeat-use workflow, pricing evaluation, macro promise preservation, next evidence path. | `scripts/check-cycle-record.mjs` product-level verdict fields; `scripts/check-data-contracts.mjs` public-test/subscription/paid-proof/external-proof/sellable-boundary/paid-onboarding UI terms. | R14 exposes paid onboarding steps while keeping payment and SLA blocked. |
| `skills/product-level-sellability/SKILL.md` | Before writing `sellable_status` or `allowed_to_stop`. | product feedback readiness, repeat-use readiness, pricing evaluation readiness, macro promise preservation. | `scripts/check-cycle-record.mjs` requires product-level verdict fields. | R14 keeps sellability blocked while payment approval and staffed SLA are missing. |
| `skills/paid-onboarding-boundary/SKILL.md` | Before adding checkout-like, paid onboarding, or subscription-start surfaces. | onboarding status, payment blocked status, SLA blocked status, non-advice acknowledgement. | `scripts/check-data-contracts.mjs` paid onboarding checks plus cycle receipts. | R14 turns paid onboarding into a reusable blocked-boundary skill instead of a one-off UI rule. |
