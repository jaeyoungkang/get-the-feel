...
if (data.meta.candidate_id !== "r13-operator-sla-radar") {
  throw new Error("quality gate must target r13-operator-sla-radar");
}
...
if (data.pricing_hypothesis?.paid_service_status !== "not_launched") {
  throw new Error("pricing hypothesis must remain not_launched for R13");
}
if (data.update_sla?.paid_sla_status !== "operator_boundary_candidate") {
  throw new Error("paid SLA must remain operator_boundary_candidate for R13");
}

if (!data.customer_proof || data.customer_proof.proof_status !== "operator_sla_candidate") {
  throw new Error("R13 must classify customer proof as operator_sla_candidate");
}
if (!data.customer_proof.proof_boundary || !data.customer_proof.proof_boundary.includes("sellability denied until external customer/payment evidence exists outside localStorage")) {
  throw new Error("R13 must include customer proof boundary denying sellability until external evidence exists");
}
if (!data.feedback_surface || data.feedback_surface.capture_storage !== "localStorage.ai-bottleneck-r13-operator-sla-context") {
  throw new Error("R13 must include operator-SLA context storage path");
}
if (!Array.isArray(data.feedback_surface.repeat_use_workflows) || data.feedback_surface.repeat_use_workflows.length < stageIds.size) {
  throw new Error("R13 must include repeat-use workflows for displayed stages");
}
if (!Array.isArray(data.feedback_surface.pricing_choices) || data.feedback_surface.pricing_choices.length < 2) {
  throw new Error("R13 must include pricing choices");
}
...
for (const blocker of ["real customer capture", "payment approval", "legal review", "paid SLA approval"]) {
  if (!data.customer_proof.external_blockers?.includes(blocker)) {
    throw new Error(`R13 missing external blocker: ${blocker}`);
  }
}
if (!html.includes("Macro Bottleneck Map") || !html.includes("가장 큰 병목") || !html.includes("전파 경로")) {
  throw new Error("R13 first viewport must preserve the macro bottleneck promise");
}
for (const term of ["Operator SLA Surface", "운영 SLA 경계 저장", "운영 SLA 판정", "결제 의향", "운영 SLA 경계 복사", "운영 SLA 경계 JSON 다운로드"]) {
  if (!html.includes(term)) throw new Error(`R13 operator-SLA UI missing ${term}`);
}
if (!html.includes("병목과 전파 경로") || !data.feedback_surface.summary.includes("병목과 전파 경로")) {
  throw new Error("R13 operator-SLA copy must support the macro bottleneck promise");
}
if (!data.subscription_surface || !data.subscription_surface.sample_paid_report || !data.subscription_surface.upgrade_decision) {
  throw new Error("R13 must include subscription surface sample report and upgrade decision");
}
if (!data.paid_proof_surface || !data.paid_proof_surface.packet_name || !data.paid_proof_surface.external_evidence_path) {
  throw new Error("R13 must include paid_proof_surface packet and external evidence path");
}
for (const key of ["selected_stage", "selected_snapshot", "routine", "price", "use_case", "contact_hint", "proof_boundary"]) {
  if (!data.paid_proof_surface.required_packet_fields?.includes(key)) {
    throw new Error(`R13 operator-SLA packet missing required field ${key}`);
  }
}
if (data.paid_proof_surface.payment_status !== "not_collected" || data.paid_proof_surface.legal_status !== "external_required" || data.paid_proof_surface.sla_status !== "candidate_not_launched") {
  throw new Error("R13 operator-SLA surface must refuse payment/legal/SLA completion claims");
}
if (!data.external_proof_surface || !Array.isArray(data.external_proof_surface.capture_paths) || data.external_proof_surface.capture_paths.length < 3) {
  throw new Error("R13 must include external proof capture paths");
}
for (const path of data.external_proof_surface.capture_paths) {
  if (path.status !== "external_required" || path.evidence_result !== "not_captured") {
    throw new Error(`R13 external path ${path.id} must remain external_required and not_captured`);
  }
}
if (!data.external_proof_surface.promotion_rule?.includes("outside localStorage")) {
  throw new Error("R13 promotion rule must require external evidence outside localStorage");
}
if (!data.sellable_boundary || data.sellable_boundary.current_decision !== "blocked") {
  throw new Error("R13 must keep sellable_boundary.current_decision blocked until evidence exists");
}
if (!data.sellable_boundary.decision_reason?.includes("external evidence is not captured")) {
  throw new Error("R13 sellable boundary must explain missing external evidence");
}
if (!data.sellable_boundary.pass_conditions?.some((item) => item.includes("outside localStorage"))) {
  throw new Error("R13 pass conditions must require evidence outside localStorage");
}
if (!data.sellable_boundary.blocked_conditions?.includes("local export only")) {
  throw new Error("R13 blocked conditions must include local export only");
}
if (!data.operator_sla || data.operator_sla.paid_sla_decision !== "blocked") {
  throw new Error("R13 must include blocked operator_sla decision");
}
if (data.operator_sla.staffing_status !== "not_staffed") {
  throw new Error("R13 operator_sla staffing_status must be not_staffed");
}
if (!data.operator_sla.pass_conditions?.some((item) => item.includes("named operator"))) {
  throw new Error("R13 operator_sla pass conditions must require named operator acceptance");
}
