...
const paidOnboarding = document.querySelector("#paidOnboarding");
...
function renderFeedbackSurface() {
  ...
  paidOnboarding.innerHTML = state.data.paid_onboarding.steps.map((item) => `
    <div class="item">
      <strong><span>${item.label}</span><span>${item.status}</span></strong>
      <span>${state.data.paid_onboarding.blocked_reason}</span>
    </div>
  `).join("");
...
function buildPublicSignal() {
  ...
  return {
    ...
    paid_onboarding_status: state.data.paid_onboarding.onboarding_status,
...
function buildReportText() {
  ...
  return [
    ...
    `유료 온보딩 판정: ${state.data.sellable_boundary.current_decision}`,
...
feedbackForm.addEventListener("submit", (event) => {
  ...
  localStorage.setItem("ai-bottleneck-r14-paid-onboarding-context", JSON.stringify(signal));
...
downloadSignalButton.addEventListener("click", () => {
  ...
  link.href = url;
  link.download = "ai-bottleneck-r14-paid-onboarding-intent.json";
  link.click();
