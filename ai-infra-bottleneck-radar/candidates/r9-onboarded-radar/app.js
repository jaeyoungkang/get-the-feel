...
function buildPublicSignal() {
  ...
  return {
    ...
    evidence_status: "public_test_intent_local_export",
...
feedbackForm.addEventListener("submit", (event) => {
  ...
  localStorage.setItem("ai-bottleneck-r8-public-test-signal", JSON.stringify(signal));
  feedbackStatus.textContent = "테스트 의사가 저장됐다. 복사나 다운로드로 인터뷰/대기자 증거 후보를 외부로 옮길 수 있다.";
});
...
downloadSignalButton.addEventListener("click", () => {
  ...
  link.href = url;
  link.download = "ai-bottleneck-r8-public-test-signal.json";
  link.click();
  URL.revokeObjectURL(url);
  exportStatus.textContent = "테스트 신호 JSON을 다운로드했다.";
});
