...
function buildPublicSignal() {
  ...
  return {
    ...
    product_surface: "subscription_ready_local_candidate",
    ...
    evidence_status: "subscription_intent_local_export",
...
function buildReportText() {
  ...
  return [
    ...
    `구독 의사: ${state.selectedPrice}`,
    `샘플 유료 리포트: ${state.data.subscription_surface.sample_paid_report}`,
...
feedbackForm.addEventListener("submit", (event) => {
  ...
  localStorage.setItem("ai-bottleneck-r9-subscription-context", JSON.stringify(signal));
  feedbackStatus.textContent = "월간 레이더가 저장됐다. 샘플 리포트와 구독 의사를 복사하거나 다운로드할 수 있다.";
});
...
downloadSignalButton.addEventListener("click", () => {
  ...
  link.href = url;
  link.download = "ai-bottleneck-r9-subscription-intent.json";
  link.click();
  URL.revokeObjectURL(url);
  exportStatus.textContent = "구독 의사 JSON을 다운로드했다.";
});
