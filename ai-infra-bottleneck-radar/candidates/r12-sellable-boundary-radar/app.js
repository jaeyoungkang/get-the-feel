...
function buildReportText() {
  ...
  return [
    ...
    `판매 가능 판정: ${state.data.sellable_boundary.current_decision}`,
...
feedbackForm.addEventListener("submit", (event) => {
  ...
  localStorage.setItem("ai-bottleneck-r12-sellable-boundary-context", JSON.stringify(signal));
...
downloadSignalButton.addEventListener("click", () => {
  ...
  link.href = url;
  link.download = "ai-bottleneck-r12-sellable-boundary-intent.json";
  link.click();
