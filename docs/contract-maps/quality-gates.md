...
| Surface audit | `npm run mc:audit-surface` | untagged user-facing surfaces after bootstrap |
...
For agentic-base itself, use:

```bash
npm run quality:check
```

This keeps the template valid without requiring product-specific Story Chain
content.
