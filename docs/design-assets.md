...
| Trainer data embed | `public/legacy/c4-3/data.js` generated from `assets/content/*.json` |
...
- `npm run ui:check`와 `npm run quality:check`가 통과한다.
- `public/legacy/c4-3/data.js`는 `npm run content:sync-legacy`로 생성되며,
  `npm run content:check`가 정본 콘텐츠와의 동등성을 확인한다.
- `npm run legacy:check`가 기존 트레이너의 단서 차단, 산출 모드, 약점
  통계, iframe bridge의 핵심 anchor를 확인한다.
