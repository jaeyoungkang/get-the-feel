#!/usr/bin/env bash
# get-the-feel — trainer snapshot을 GitHub Pages로 배포 (approve-5 재결정, 2026-06-11)
# 사용: tools/deploy/deploy-pages.sh <snapshot-id>   예) c4-3
# 배포 저장소: github.com/jaeyoung2026/get-the-feel (공개 — 배포 산출물만, 방법론·기록 비공개 유지)
set -euo pipefail
CAND="${1:?snapshot-id 필요 (예: c4-3)}"
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SRC="$ROOT/archive/training-snapshots/$CAND"
[ -f "$SRC/index.html" ] || { echo "snapshot 없음: $SRC"; exit 1; }
