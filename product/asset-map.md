...
- `last_cycle_contribution`: c2-1 — 결합 가설 검증(C2 기준 1 카운트 충족), 합성 과신 반증 → R10.
...
- `last_cycle_contribution`: c2-1 — get.json + phrasal-up.json (cross-axis 12, verb_label 필드 신설). 코퍼스 4파일 57문장 — 기준 2(100+)까지 확장 필요.
...
- `last_cycle_contribution`: c2-1 — R9 첫 실전 검증(혼합 출제 작동 실증) + R10 신설(합성 반례) + verb-choice 문법 단서 메모.
...
- `last_cycle_contribution`: c2-1 — G7(get 도달 메타포)·G8(합성 시각화 + verb_label 명시 필드) 신설.
...
- `last_cycle_contribution`: c2-1 — 다파일 후보 매핑(CONTENT_ALL), 유형 시스템·adversarial-review 게이트 첫 실전. 기록: SVG 캡션 검사는 1회 결함이라 보류.
