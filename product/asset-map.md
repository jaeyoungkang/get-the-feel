...
- `last_cycle_contribution`: bootstrap — 계약 최초 잠금.

## 2. 감각 콘텐츠 코퍼스 + 데이터 계약 (핵심 자산)

- `asset_path`: `assets/content/` (스키마 정본: `assets/content/CONTRACT.md` — c1-1에서 생성)
...
- `mechanical_or_review_gate`: 데이터 계약 스키마 검사(출처 필드 필수) + 훈련/전이 분리 검사 — `tools/verdict/` (c1-1에서 생성).
- `last_cycle_contribution`: 없음 (c1-1부터).

## 3. 훈련 설계 규칙

- `asset_path`: `assets/training-design.md` (c1-1에서 생성)
...
- `asset_path`: `assets/ux-grammar.md` (c1-1에서 생성)
...
- `asset_path`: `tools/verdict/` (c1-1에서 생성)
