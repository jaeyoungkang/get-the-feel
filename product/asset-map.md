...
- `last_cycle_contribution`: c1-1 — V1 부분 검증+반증, V3 후보 추가, 빈틈 6·7 운영 확인.
...
- `last_cycle_contribution`: c1-1 — have.json (sense 2 + boundary_ko, 문항 15) + CONTRACT 규칙 8항.
...
- `last_cycle_contribution`: c1-1 — R1~R8 (정답 셔플, 근접 오개념 오답, 경계 문항, 전이 패턴 거리, 출처-그림 일치, 콘텐츠 공급=재방문, 산출 한계, 첫 세션 화면).
...
- `last_cycle_contribution`: c1-1 — G1~G6 (공간 메타포 어휘, 라벨 명시 필드, 반복 피로, 오답 긴장, 표면 어휘, 정직 라벨).
...
- `last_cycle_contribution`: c1-1 — 6검사군 신설. c1-2 추가 예정: 보기 셔플 검사, 라벨 필드 검사.
