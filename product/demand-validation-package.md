# 수요 검증 패키지 (C3 Readiness 확정 — local_ready 전제)
...
| `최소_artifact` | 배포된 c2-3 (8감각 120문장, 출처 strong, 해석 피드백, Day 추적, 세션 export) + blind task 안내문 4질문 |
...
| `검증_위치` | 타인 blind 결과 → `candidates/c2-3/cycle-record.md` append(덮어쓰기 금지) / 본인 기록 → 세션 export JSON / 집계 → 이 파일 하단 결과 섹션 |
...
- `as_of`: 콘텐츠는 코퍼스 버전 v1(8감각 120문장, 2026-06-13 strong 승격) 기준 고정. 갱신 시 버전 표기.
- `provenance`: 전 sense `source_refs` + `validation: human/strong` (sources.md, 웹 검증 + approve-4).
- `상태 구분`: "새 문장 N개 남음"은 *이 브라우저 localStorage 출제 이력* 기준 (서버 없음 — 기기·브라우저별 독립). ⓘ에 명시됨.
- `소진 처리`: 120문항 출제 완료 시 "한 바퀴 완주 → 복습 모드"로 정직 전환 (빈 화면 금지). 진짜 "매일 새 문장"의 지속은 코퍼스 확장(다음 동사 축)에 의존 — 이건 출시 후 운영 약속이며, 현재는 7일치까지 보장.
