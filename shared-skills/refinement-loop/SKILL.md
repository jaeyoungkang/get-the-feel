...
**핵심 형태**
- **결과물은 폐기 가능, 배움은 산출물로 회수** — 매 사이클 zero에서 시작. *배움(learning)은 마크다운 누적 문서로 두지 않는다.* 매 사이클 종료 시 learning은 **작동하는 산출물**(코드·스킬·계약·프롬프트·데이터 스키마 등)로 즉시 *회수*된다. learnings.md 같은 텍스트 ledger는 *임시 적재함*일 뿐이며, 회수 안 된 learning은 사이클 종료 못 함. 회수된 learning은 코드/스킬 변경 자체가 됨 → 운영 흔적은 commit log가 잡는다.
...
**도메인이 결정하는 것**
- learning 회수처 — 어떤 산출물(코드·스킬·계약·프롬프트·스키마)로 learning이 회수되는가의 매핑. 임시 적재함 형식은 자유이나 *회수처 명시*는 강제
- 외부 검증 신호 (사람·에이전트·기계)
- 상한 N과 조기 종료 조건
- "사이클" 단위의 크기
- 사다리 단계 정의 (위 "사이클 형식 사다리" 섹션 참조)

베이스는 패턴을 *옵션으로* 제공하며 강제하지 않는다. Spiral Loop 적용 여부는 도메인이 결정한다. **단 Spiral Loop를 적용하기로 한 도메인은 (a) Forward Momentum 규칙(사다리 명시 + 매 사이클 다음 단계 명시 + 긍정형 통과 기준 1개)과 (b) Learning 회수 원칙(텍스트 누적 금지, 매 사이클 산출물로 회수)을 함께 적용한다.**
