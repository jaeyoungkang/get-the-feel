...
**핵심 형태**
- **결과물은 폐기 가능, 배움은 append-only** — 매 사이클 zero에서 시작, 고정 섹션의 learnings ledger에 배움 누적
...
**도메인이 결정하는 것**
- learnings ledger 형식 (섹션 구성, 파일 위치)
- 외부 검증 신호 (사람·에이전트·기계)
- 상한 N과 조기 종료 조건
- "사이클" 단위의 크기

베이스는 패턴을 *옵션으로* 제공하며 강제하지 않는다. Spiral Loop 적용 여부는 도메인이 결정한다.
